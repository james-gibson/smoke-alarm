...
Feature: OAuth Mock Redirect Endpoint
  ...
  Scenario: mock redirect endpoint starts when mock_redirect.enabled is true
    ...
    Then a listener is bound on "127.0.0.1:28877"
  ...
  Scenario: allow mode returns 200 for any callback request
    Given the mock redirect endpoint is running in "allow" mode on "127.0.0.1:28877"
    When a GET request is sent to "http://127.0.0.1:28877/oauth/callback?state=s1&callback_id=cb-1"
  ...
  Scenario: ACP target with OAuth auth completes handshake in allow mode
    Given the mock redirect endpoint is running in "allow" mode on "127.0.0.1:28877"
    And the hosted ACP server is running on "127.0.0.1:28191"
  ...
  Scenario: fail mode returns a non-200 status for any callback request
    Given the mock redirect endpoint is running in "fail" mode on "127.0.0.1:28877"
    When a GET request is sent to "http://127.0.0.1:28877/oauth/callback?state=s2&callback_id=cb-fail-1"
  ...
  Scenario: HURL test exercises the mock redirect callback endpoint directly
    ...
    When the probe for target "acp-oauth-fail-sample" runs
    Then the HURL test "oauth-mock-redirect-callback-fail" sends a GET to "http://127.0.0.1:28877/oauth/callback?state=s2&callback_id=cb-fail-1"
